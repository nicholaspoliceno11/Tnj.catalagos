    ...
    if (token) {
      try {
        ...
        if (imageErrors?.length) {
          showAlert(
            `Catálogo publicado (${activeCount} ativo(s) de ${productCount})! ${imagesUploaded} foto(s) enviada(s), ${imageErrors.length} falhou(aram). O site atualiza sozinho em 1–2 min.`,
            "success"
          );
        } else {
          showAlert(
